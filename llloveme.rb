...
use_bpm 113
set_mixer_control! amp: 1, pre_amp: 1, hpf: 30, lpf_slide: 16
...
live_loop :kick do
  ...
  sample :bd_808, cutoff: 130, amp: amp_1,
    compress: 1, pre_amp: 10, slope_above: 0.7, slope_below: 3 if (ring true, true).look
...
live_loop :fast do
  amp = 0
  sample :elec_bong, amp: amp, pan: 0.1, slice: 4, num_slices: 4 if one_in(3)
  sleep 0.25
end

live_loop :fss do
  amp = 0 #0.6
  sample :loop_3d_printer, amp: amp, slice: 0.2, pan: 0.3
  sleep 8
end
...
live_loop :myfm do
  tick
  amp = 1
  with_fx :slicer, mix: 0.5, phase: 0.25, phase_offset: 0.25, pulse_width: 0.25, smooth: 0.1, wave: 1 do
    with_swing shift: 0.0 do #0.175 do
...
live_loop :myfm2 do
  tick
  amp = 0
  with_fx :reverb, mix: 0.5, damp: 0.25, room: 0.6 do
    with_synth :mod_fm do
      s3 = (knit :b4, 4, :r, 6 , :a4, 2,  :g4, 2, :e4, 2)
      s4 = (knit :r, 10, :e4, 2,  :g4, 2, :e5, 2)
      s5 = (knit :c5, 10, :f4, 2,  :a4, 2, :f4, 2)
      s = play (s3 + s4 + s5 + s5 + s5).look, amp: amp, pan: -0.2, sustain: 1,
        release: 1,
        mod_range: 0.25,
        mod_phase: 0.5,
        cutoff: 90
      sleep 0.7
      control s, mod_phase: 0.175, mod_range: -1
      sleep 0.3
    end
  end
end
